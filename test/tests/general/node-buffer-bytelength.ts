...
export default function() {
  ...
  assert.equal(Buffer.byteLength('', <any>'HeX'), 0);
  ...
  assert.equal(Buffer.byteLength('𠝹𠱓𠱸', <any>'UTF8'), 12);
  // without an encoding, utf8 should be assumed
  assert.equal(Buffer.byteLength('hey there'), 9);
  assert.equal(Buffer.byteLength('𠱸挶νξ#xx :)'), 17);
  assert.equal(Buffer.byteLength('hello world', <any>''), 11);
  // it should also be assumed with unrecognized encoding
  assert.equal(Buffer.byteLength('hello world', <any>'abc'), 11);
  assert.equal(Buffer.byteLength('ßœ∑≈', <any>'unkn0wn enc0ding'), 10);
  ...
  ['ucs2', 'ucs-2', 'utf16le', 'utf-16le'].forEach(function(encoding) {
    assert.equal(24, Buffer.byteLength('Il était tué', <BufferEncoding>encoding));
  });
};
